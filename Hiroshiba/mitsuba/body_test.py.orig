...
def is_angles_match_limits(left_shoulder_angles, right_shoulder_angles, left_elbow_angle, right_elbow_angle, left_hip_angles, right_hip_angles, left_knee_angle, right_knee_angle):
    
    def angle_detection_generator():
        yield left_shoulder_angles['p'] >= shoulder_limits['p'][0]
        yield left_shoulder_angles['p'] <= shoulder_limits['p'][1] 
        yield left_shoulder_angles['y'] >= shoulder_limits['y'][0]
        yield left_shoulder_angles['y'] <= shoulder_limits['y'][1]
        yield left_shoulder_angles['r'] >= shoulder_limits['r'][0]
        yield left_shoulder_angles['r'] <= shoulder_limits['r'][1]
        yield left_elbow_angle >= elbow_limits[0]
        yield left_elbow_angle <= elbow_limits[1]
        yield right_elbow_angle >= elbow_limits[0]
        yield right_elbow_angle <= elbow_limits[1]
        yield left_hip_angles['p'] >= hip_limits['p'][0]
        yield left_hip_angles['p'] <= hip_limits['p'][1]
        yield left_hip_angles['y'] >= hip_limits['y'][0]
        yield left_hip_angles['y'] <= hip_limits['y'][1]
        yield left_hip_angles['r'] >= hip_limits['r'][0]
        yield left_hip_angles['r'] <= hip_limits['r'][1]
        yield right_hip_angles['p'] >= hip_limits['p'][0]
        yield right_hip_angles['p'] <= hip_limits['p'][1]
        yield right_hip_angles['y'] >= hip_limits['y'][0]
        yield right_hip_angles['y'] <= hip_limits['y'][1]
        yield right_hip_angles['r'] >= hip_limits['r'][0]
        yield right_hip_angles['r'] <= hip_limits['r'][1]
        yield left_knee_angle >= knee_limits[0]
        yield left_knee_angle <= knee_limits[1]
        yield right_knee_angle >= knee_limits[0]
        yield right_knee_angle <= knee_limits[1]

    for angle_match in angle_detection_generator():
        if not angle_match:
            return False
...
while (index < num_images):
    ...
    left_shoulder_angles['p'] += randint(-1, 2)
    left_shoulder_angles['y'] += randint(-1, 2)
    left_shoulder_angles['r'] += randint(-1, 2)
    right_shoulder_angles['p'] += randint(-1, 2)
    right_shoulder_angles['y'] += randint(-1, 2)
    right_shoulder_angles['r'] += randint(-1, 2)
    left_elbow_angle += randint(-1, 2)
    right_elbow_angle += randint(-1, 2)
    left_hip_angles['p'] += randint(-1, 2)
    left_hip_angles['y'] += randint(-1, 2)
    left_hip_angles['r'] += randint(-1, 2)
    right_hip_angles['p'] += randint(-1, 2)
    right_hip_angles['y'] += randint(-1, 2)
    right_hip_angles['r'] += randint(-1, 2)
