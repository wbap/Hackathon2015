...
argvs = sys.argv
argc = len(argvs)
num_images = 500 # number of images per a movie (true)
frame_intaerval = 10 # num_images / frame_intaerval -> number of output images per a movie
num_movie = int(argvs[1])

before_index = int(argvs[2]) # index of movie which i had rendered  

# settings
lower_body_flag = True 
first_person_view_flag = False 
lr_flip_flag = False 
...
while movie_index < num_movie:
    ...
    target_list = ['other_mit_body_v2_%05i_%05i' % (movie_index+1, i+1) + '.png' for i in xrange(num_images) if ((i+1) % frame_intaerval == 0)]
    if not generate_json_flag:
        json_list = ['other_mit_body_v2_%05i_%05i' % (movie_index+1, i+1) + '.json' for i in xrange(num_images) if ((i+1) % frame_intaerval == 0)]
    
    if generate_json_flag:
        # Normal run (generating json & png images)
        while (index < num_images):
            destination = 'other_mit_body_v2_%05i_%05i' % (movie_index+1, index+1)
            scene = Scene()
            ...
            temp1 = left_shoulder_angles['p'] + randint(-1, 2) * step_length
            if ((temp1 < shoulder_limits['p'][0]) or (temp1 > shoulder_limits['p'][1])):
                continue
        
            temp2 = left_shoulder_angles['y'] + randint(-1, 2) * step_length
            if ((temp2 < shoulder_limits['y'][0]) or (temp2 > shoulder_limits['y'][1])):
                continue
        
            temp3 = left_shoulder_angles['r'] + randint(-1, 2) * step_length
            if ((temp3 < shoulder_limits['r'][0]) or (temp3 > shoulder_limits['r'][1])):
                continue
        
            temp4 = right_shoulder_angles['p'] + randint(-1, 2) * step_length
            if ((temp4 < shoulder_limits['p'][0]) or (temp4 > shoulder_limits['p'][1])):
                continue
        
            temp5 = right_shoulder_angles['y'] + randint(-1, 2) * step_length
            if ((temp5 < shoulder_limits['y'][0]) or (temp5 > shoulder_limits['y'][1])):
                continue
        
            temp6 = right_shoulder_angles['r'] + randint(-1, 2) * step_length
            if ((temp6 < shoulder_limits['r'][0]) or (temp6 > shoulder_limits['r'][1])):
                continue
                    
            temp7 = left_elbow_angle + randint(-1, 2) * step_length
            if ((temp7 < elbow_limits[0]) or (temp7 > elbow_limits[1])):
                continue
        
            temp8 = right_elbow_angle + randint(-1, 2) * step_length
            if ((temp8 < elbow_limits[0]) or (temp8 > elbow_limits[1])):
                continue
            
            if lower_body_flag:
                temp9 = left_hip_angles['p'] + randint(-1, 2) * step_length
                if ((temp9 < hip_limits['p'][0]) or (temp9 > hip_limits['p'][1])):
                    continue
        
                temp10 = left_hip_angles['y'] + randint(-1, 2) * step_length
                if ((temp10 < hip_limits['y'][0]) or (temp10 > hip_limits['y'][1])):
                    continue
        
                temp11 = left_hip_angles['r'] + randint(-1, 2) * step_length
                if ((temp11 < hip_limits['r'][0]) or (temp11 > hip_limits['r'][1])): 
                    continue
        
                temp12 = right_hip_angles['p'] + randint(-1, 2) * step_length
                if ((temp12 < hip_limits['p'][0]) or (temp12 > hip_limits['p'][1])):
                    continue
        
                temp13 = right_hip_angles['y'] + randint(-1, 2) * step_length
                if ((temp13 < hip_limits['y'][0]) or (temp13 > hip_limits['y'][1])):
                    continue
        
                temp14 = right_hip_angles['r'] + randint(-1, 2) * step_length
                if ((temp14 < hip_limits['r'][0]) or (temp14 > hip_limits['r'][1])):
                    continue
                
                temp15 = left_knee_angle + randint(-1, 2) * step_length
                if ((temp15 < knee_limits[0]) or (temp15 > knee_limits[1])):
                    continue
        
                temp16 = right_knee_angle + randint(-1, 2) * step_length
            ...
            if ((body.is_collision_body_parts() == False) and (departure_flag == False)):
                ...
                if ((index+1) % frame_intaerval == 0): # every "frame_intaerval", the image was rendered
                    # Add body parts
                    for prop in body.get_property_list():
                        scene.addChild(pmgr.create(prop))
        
                    json_data = body.constr_json_data()                            
                    with open('other_mit_body_v2_%05i_%05i.json' % (movie_index+1, index+1), 'w') as f:
                    ...
                    job = RenderJob('myRenderJob' + str(movie_index) + '_' + str(index), scene, queue)
        ...
        for index, target_json in enumerate(json_list):
            ...
            # Create a render job and insert it into the queue
            job = RenderJob('myRenderJob' + str(movie_index) + '_' + str((index+1) * frame_intaerval-1), scene, queue)
            job.start()
    ...
    while True:
        ...
        for i, flag in enumerate(flag_list):
            if not flag:
                # open json file
                with open('other_mit_body_v2_%05i_%05i.json' % (movie_index+1, ((i+1) * frame_intaerval))) as f:
                    json_data = json.load(f)
                # Set angle parameters from json data (not json file!)
                body.set_from_json_data(json_data)
                destination = 'other_mit_body_v2_%05i_%05i' % (movie_index+1, ((i+1) * frame_intaerval))
        ...
        re_rendering_number += 1

        if re_rendering_number > 2:
            fail_flag = True
            break
    ...
    if fail_flag:
        print 'can not rendering all images in this movie sequence'
        remove_targets = glob.glob('other_mit_body_v2_%05i_*' % (movie_index+1))
        for file_name in remove_targets:
            os.remove(file_name)
    else:
        movie_index+=1
