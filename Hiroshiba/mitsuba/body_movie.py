...
while movie_index < num_movie:
    ...
    target_list = ['mit_body_v2_%05i_%05i' % (movie_index+1, i+1) + '.png' for i in xrange(num_images) if ((i+1) % frame_intaerval == 0)]
    if not generate_json_flag:
        json_list = ['mit_body_v2_%05i_%05i' % (movie_index+1, i+1) + '.json' for i in xrange(num_images) if ((i+1) % frame_intaerval == 0)]
    
    if generate_json_flag:
        # Normal run (generating json & png images)
        while (index < num_images):
            destination = 'mit_body_v2_%05i_%05i' % (movie_index+1, index+1)
            scene = Scene()
            ...
            if ((body.is_collision_body_parts() == False) and (departure_flag == False)):
                ...
                if ((index+1) % frame_intaerval == 0): # every "frame_intaerval", the image was rendered
                    ...
                    with open('mit_body_v2_%05i_%05i.json' % (movie_index+1, index+1), 'w') as f:
    ...
    while True:
        ...
        for i, flag in enumerate(flag_list):
            if not flag:
                # open json file
                with open('mit_body_v2_%05i_%05i.json' % (movie_index+1, ((i+1) * frame_intaerval))) as f:
                    json_data = json.load(f)
                # Set angle parameters from json data (not json file!)
                body.set_from_json_data(json_data)
                destination = 'mit_body_v2_%05i_%05i' % (movie_index+1, ((i+1) * frame_intaerval))
    ...
    if fail_flag:
        ...
        remove_targets = glob.glob('mit_body_v2_%05i_*' % (movie_index+1))
