...
"""
class syuwa_cnn_lstm_drop(FunctionSet):

    def __init__(self, num_iunits, num_hunits, num_labels):
        super(syuwa_cnn_lstm_drop, self).__init__(
    ...
    def forward_one_step(self, x_vis, x_dep, train_label, c, h, volatile=False):
        ...
        h_in = self.i2h(F.dropout(lstm_input, ratio=0.75, train=not volatile)) + self.h2h(h)
        c, h = F.lstm(c, h_in)
