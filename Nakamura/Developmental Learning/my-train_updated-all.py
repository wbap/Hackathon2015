...
def create_result_dir(args):
    if args.restart_from is None:
        result_dir = 'results/' + os.path.basename("VGG_updated").split('.')[0]
...
class VGG_mini2(FunctionSet):
    def __init__(self):
        super(VGG_mini2, self).__init__(
            ...
            conv1_3=F.Convolution2D(64, 256, 3, stride=1, pad=1),
...
class VGG_mini3(FunctionSet):
    def __init__(self):
        super(VGG_mini3, self).__init__(
            conv1_1=F.Convolution2D(3, 64, 3, stride=1, pad=1),
            conv1_2=F.Convolution2D(64, 64, 3, stride=1, pad=1),
            conv1_3=F.Convolution2D(64, 64, 3, stride=1, pad=1),
            conv1_4=F.Convolution2D(64, 256, 3, stride=1, pad=1),
...
class VGG_mini4(FunctionSet):
    def __init__(self):
        super(VGG_mini4, self).__init__(
            ...
            conv1_2=F.Convolution2D(64, 64, 3, stride=1, pad=1),
            conv1_3=F.Convolution2D(64, 64, 3, stride=1, pad=1),
            conv1_4=F.Convolution2D(64, 64, 3, stride=1, pad=1),
            conv1_5=F.Convolution2D(64, 256, 3, stride=1, pad=1),
...
def train(n_epoch, model, N, N_test, batchsize, x_train, y_train, x_test, y_test, **kwargs):
    ...
    for epoch in six.moves.range(1, n_epoch + 1):
        ...
        for i in six.moves.range(0, N, batchsize):
            ...
            # print "time:{},loss:{},acc:{}".format(time,float(loss.data) * len(y_batch), float(acc.data) * len(y_batch))
